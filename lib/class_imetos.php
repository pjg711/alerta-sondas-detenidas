...
class IMETOS
{
    ...
    function __construct($server=null,$database=null,$username=null,$password=null)
    {
        global $db_link2;
        $this->rowCount=0;
        if(!($db_link2 = new PDO('mysql:host='.$server.';dbname='.$database.';charset=utf8',$username,$password)))
        {
    ...
    public function sql_select($query, &$rv)
    {
        ...
        {
            $db_link2->query("SET NAMES 'utf8'");
        }
        $rv = $db_link2->prepare($query);
        if(!$rv->execute())
        {
            return false;
        }
        if($last_id=$db_link2->lastInsertId())
        {
            return $last_id;
        }
        if($rv->rowCount())
...
class Station
{
    ...
    public function instantSensorData(Array $filterById = null)
    {
        ...
        $query = "SELECT
                ...
                FROM
                    `" . DBT_PREFIX . "sensor_data_retrieve_info_{$tableSuffix}`
                WHERE
                    `out_of_range` <> 1
                AND
                    `f_read_time` = (SELECT  MAX(`f_read_time`) FROM `" . DBT_PREFIX . "sensor_data_retrieve_info_{$tableSuffix}` WHERE `f_station_code` = {$this->_fStationCode} AND `out_of_range` <> 1)
                AND
                    `f_station_code` = {$this->_fStationCode}
                AND
                    `f_sensor_code` IN (SELECT `f_sensor_code` FROM `" . DBT_PREFIX . "sensor_info` WHERE {$filterById} `enable_sensor` = 1 AND `f_station_code` = {$this->_fStationCode})
            ";
