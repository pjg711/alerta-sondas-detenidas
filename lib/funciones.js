...
function seleccionar_sensores_todos(estacion){
    ...
    for(i=0;i<objs.length;i++){
        ...
        if(objs[i].id=='sensor_todos'){
            var valor=objs[i].checked;
        }
        if(objs[i].id!='sensor_todos'){
...
function estacion_activa(activar,estacion){
    ...
    var obj=document.getElementById(estacion);
    var objs=obj.getElementsByTagName('input');
    var objs2=obj.getElementsByTagName('label');
    console.log(activar.checked);
    if(activar.checked){
        //activo
        var valor=false;
        var bcolor='#000000';
    ...
    for(i=0;i<objs.length;i++){
        if(objs[i].id!='activar')
        {
            objs[i].disabled=valor;
        }
    }
    for(i=0;i<objs2.length;i++){
        objs2[i].style.color=bcolor;
    }
}